...
export const createRegion = async (region: Omit<Region, 'id' | 'created_at'>) => {
  return logApiCall('createRegion', async () => {
    try {
...
export const updateRegion = async (id: string, region: Partial<Omit<Region, 'id' | 'created_at'>>) => {
  return logApiCall(`updateRegion:${id}`, async () => {
    try {
      const { data, error } = await supabase
        .from('regions')
        .update(region)
...
export const deleteRegion = async (id: string) => {
  return logApiCall(`deleteRegion:${id}`, async () => {
    try {
...
export const archiveRegion = async (id: string) => {
  return logApiCall(`archiveRegion:${id}`, async () => {
    try {
